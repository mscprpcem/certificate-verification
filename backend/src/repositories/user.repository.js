...
class UserRepository {
  ...
  async updateXpAndLevel(id, xp, level) {
    return await dbRun("UPDATE users SET xp = ?, level = ? WHERE id = ?", [xp, level, id]);
  }
  ...
  async getAdminUsersDirectory() {
    return await dbAll(`
      SELECT u.id, u.name, u.email, u.role, u.bio, u.headline, u.profile_photo, u.linkedin_url, u.github_url, u.skills, u.xp, u.level, u.created_at,
